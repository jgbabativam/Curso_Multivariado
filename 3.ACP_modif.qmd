...
if (file.exists(dest)) {
  ...
  # Varianza explicada acumulada por canal
  var_exp_r <- cumsum(pca_r$sdev^2) / sum(pca_r$sdev^2)
  var_exp_g <- cumsum(pca_g$sdev^2) / sum(pca_g$sdev^2)
  var_exp_b <- cumsum(pca_b$sdev^2) / sum(pca_b$sdev^2)
}
```
...
fviz_pca_var(pca_ecv,
             col.var = pca_ecv$var$coord[, 1],
             gradient.cols = c("#D6EAF8", "#2980B9", "#1A5276"),
             repel = TRUE,
             title = "Plano 1-2 — coloreado por correlación con Dim 1") +
  theme_bw()
```
...
#| message: false
#| warning: false
fviz_pca_var(pca_ecv, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE,
             title = "Plano 1-2 — coloreado por contribución") +
  theme_bw()
```
...
#| message: false
#| warning: false
fviz_pca_var(pca_ecv, col.var = "cos2",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE,
             title = "Plano 1-2 — coloreado por cos²") +
  theme_bw()
```
...
#| fig-width: 9
#| fig-height: 4.5
fviz_contrib(pca_ecv, choice = "var", axes = 3,
             fill = "#8E44AD", color = "#8E44AD",
             title = "Contribuciones a la Dim 3") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
```
...
## Dimensión 3: más allá del factor tamaño {style="font-size: 90%;"}
...
#| message: false
#| warning: false
fviz_pca_var(pca_ecv, axes = c(1, 3),
             col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE,
             title = paste0("Plano 1-3 — Variables (",
                            round(eig_ecv[3, 2], 1), "% adicional en Dim 3)")) +
  theme_bw()
```

### Departamentos (plano 1-3)
...
#| message: false
#| warning: false
fviz_pca_ind(pca_ecv, axes = c(1, 3),
             col.ind = "cos2", repel = TRUE, labelsize = 3.5,
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             title = "Plano 1-3 — Departamentos de Colombia") +
  theme_bw()
```
...
::: {.fragment}
La Dim 3 captura una **dimensión residual** no cubierta por el combo completo de infraestructura (Dim 1) ni por el contraste conectividad-servicios básicos (Dim 2). Las variables con alta contribución a Dim 3 señalan aspectos de calidad de vida que operan de forma **relativamente independiente** del nivel general.
:::

---

## Lectura del plano: regiones de Colombia {style="font-size: 85%;"}

::: {.incremental}
- **Dim 1 (eje horizontal):** **factor tamaño**. Bogotá D.C. y el Eje Cafetero tienen coordenadas altas (derecha); Chocó, La Guajira y territorios amazónicos están a la izquierda.
- **Dim 2 (eje vertical):** contraste entre departamentos con **alta cobertura de servicios básicos** (acueducto, alcantarillado — arriba) vs. **alta conectividad digital** (internet, computador — abajo).
- **Variables en el mismo cuadrante:** están positivamente correlacionadas (p.ej., `internet` y `computador`).
- **"Cuadrantes opuestos" con factor tamaño dominante:** dos variables en Q1 y Q4 (o en Q2 y Q3) pueden estar **muy correlacionadas positivamente** (~0.73) aunque aparezcan en lados opuestos del eje 2 — porque **ambas apuntan al mismo lado del factor tamaño (eje 1)**. El ángulo entre vectores solo equivale directamente a la correlación cuando ningún eje está dominado por un factor tamaño.
- **Correlación negativa real:** requiere que los vectores apunten a cuadrantes diagonalmente opuestos (Q1 vs Q3, o Q2 vs Q4) **y** que el factor tamaño no sea dominante.
:::
...
## ¿Qué es un píxel? {style="font-size: 90%;"}
...
## El tensor RGB {style="font-size: 90%;"}
...
## Varianza explicada acumulada — los tres canales {style="font-size: 90%;"}
...
## Varianza explicada acumulada — los tres canales
...
#| warning: false
if (exists("var_exp_r")) {
  max_comp <- min(150, length(var_exp_r))

  df_rgb <- bind_rows(
    data.frame(comp = seq_len(max_comp),
               var_acum = var_exp_r[seq_len(max_comp)] * 100, Canal = "R"),
    data.frame(comp = seq_len(max_comp),
               var_acum = var_exp_g[seq_len(max_comp)] * 100, Canal = "G"),
    data.frame(comp = seq_len(max_comp),
               var_acum = var_exp_b[seq_len(max_comp)] * 100, Canal = "B")
  )
...
3. Aplique el ACP usando PCA() de FactoMineR. Compare:
  - `scale.unit = TRUE` (sobre $R$)
  - `scale.unit = FALSE` (sobre $S$)
...
Identifique:
  ...
  - Una variable con bajo $cos^2$ en el plano 1-2 (mal representada).

6. Construya el **plano de individuos** con fviz_pca_ind() coloreando por $cos^2$.
Identifique:
...
10. **Reto:** Clasifique los departamentos en **4 grupos**.
