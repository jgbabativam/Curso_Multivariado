...
library(pacman)
p_load(here, tidyr, dplyr, ggplot2, FactoMineR, factoextra, gt)

confianzaJUstcia <- read.csv2(here::here("datos/ConfianzaJusticia.csv"))
...
confianzaJUstcia_filtrado <- confianzaJUstcia[
  rownames(confianzaJUstcia) %in% paises_interes, ]
```
...
conf_tot <- cbind(confianzaJUstcia_filtrado,
                  Total = rowSums(confianzaJUstcia_filtrado))
...
perf_fil <- round(prop.table(as.matrix(confianzaJUstcia_filtrado), margin = 1), 3)
...
confpaises <- CA(as.matrix(confianzaJUstcia_filtrado), graph = FALSE)
...
library(pacman)
p_load(here, tidyr, dplyr, ggplot2)

CCLatin <- read.csv2(here::here("datos/Muestra_CC.csv"), sep = ";")
...
SUE <- read.csv2(here::here("datos/SUEsolo2012.csv"), sep = ";")
