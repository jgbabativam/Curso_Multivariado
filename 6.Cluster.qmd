...
subtitle: "Análisis de Agrupaciones (Clúster): Métodos de Partición, Jerárquicos y HCPC"
author:
...
p_load(here, tidyverse, FactoMineR, factoextra, corrplot, cluster,
       ggrepel, gt, janitor, haven, kableExtra, NbClust)
```
...
# Análisis de Agrupaciones (Clúster) {background-color="#0077b6"}

## ¿Qué es el análisis de agrupaciones (clúster)? {style="font-size: 90%;"}

Conjunto de técnicas para agrupar objetos con base en la similitud que tienen en un vector de variables $\mathbf{x}' = (x_1, \ldots, x_p)$, **sin conocer de antemano** a qué grupo pertenece cada uno.

::: {.fragment}
Un buen agrupamiento cumple dos condiciones simultáneas:

1. **Homogeneidad interna**: los objetos dentro de una misma agrupación (clúster) son lo más similares posible.
2. **Heterogeneidad externa**: las agrupaciones (clústeres) se diferencian claramente entre sí.
:::
...
## Tipos de análisis de agrupaciones (clúster) {style="font-size: 85%;"}
...
Construyen un árbol (dendrograma) agrupando sucesivamente los elementos o grupos más próximos, sin fijar de antemano el número de agrupaciones (clústeres).
...
Dividen el conjunto de objetos en un número **prefijado** $k$ de grupos, optimizando algún criterio (p. ej. minimizar la varianza intra-agrupación (intra-clúster)).

*Requieren definir $k$ antes de correr el algoritmo.*
:::
:::

## Etapas de un análisis de agrupaciones (clúster) {style="font-size: 85%;"}
...
6. Obtener la **agrupación** y decidir el número de agrupaciones (clústeres).
...
- Existen además otras medidas (p. ej. **Gower**, para mezclas de variables cuantitativas y cualitativas) que no se profundizan en este capítulo.
:::
...
Dado un número de grupos $k$, el algoritmo busca la partición que **minimiza la suma de cuadrados intra-agrupación (intra-clúster)**:
...
# Ejemplo simulado {background-color="#0077b6"}

## Datos artificiales {style="font-size: 88%;"}

Seis objetos descritos por dos variables: gasto mensual y años de educación.

```{r}
#| echo: true
library(pacman)
p_load(here, tidyverse, FactoMineR, factoextra, corrplot, cluster,
       ggrepel, gt, janitor, haven, NbClust)

datos <- data.frame(
  Individuo = c("A", "B", "C", "D", "E", "F"),
  gastos = c(5, 6, 18, 20, 25, 30),
...
```{r}
#| echo: true
mat_z <- datos_z[, c("gastoz", "educaz")]
rownames(mat_z) <- datos_z$Individuo
(distancias <- dist(mat_z))
```

::: {.fragment}
Los objetos A-B, C-D y E-F son los pares más próximos: son los primeros candidatos a fusionarse en cualquier algoritmo de agrupación.
:::
...
El número de agrupaciones (clústeres) $k$ debe fijarse **antes** de correr K-medias. Existen varios criterios para ayudar a elegirlo:
...
Grafica la suma de cuadrados intra-agrupación (intra-clúster) (WSS) contra $k$:
...
Para cada objeto $i$, compara su distancia promedio a los demás miembros de su agrupación (clúster) ($a_i$) con la distancia promedio a la agrupación (clúster) vecina más próxima ($b_i$):

$$s_i = \frac{b_i - a_i}{\max(a_i,\, b_i)}, \qquad s_i \in [-1, 1]$$

Se elige el $k$ que **maximiza** el ancho de silueta promedio $\bar{s}$. Valores cercanos a 1 indican objetos bien agrupados; cercanos a 0 o negativos, mal clasificados.

### Hartigan

**Estadístico de Hartigan**

Compara la reducción del WSS al pasar de $k$ a $k+1$ grupos, ajustada por el tamaño de la muestra $n$:

$$H(k) = \left(\frac{WSS(k)}{WSS(k+1)} - 1\right)(n - k - 1)$$

Se aumenta $k$ mientras $H(k)$ sea grande; se elige el primer $k$ en que $H(k)$ cae por debajo de un umbral (usualmente ≈10).

### Calinski-Harabasz

**Índice de Calinski-Harabasz**

Compara la dispersión entre grupos (SSB) con la dispersión dentro de los grupos (SSW), cada una ponderada por sus grados de libertad:

$$CH(k) = \frac{SSB/(k-1)}{SSW/(n-k)}$$

Se elige el $k$ que **maximiza** $CH(k)$.

:::

::: {.fragment}
::: {.callout-important}
El codo es una regla visual (subjetiva); silueta, Hartigan y Calinski-Harabasz dan un valor numérico comparable entre distintos $k$. Se recomienda usar **varios criterios** junto con el criterio del analista.
:::
:::

## ¿Cómo elegir $k$? {style="font-size: 85%;"}
...
fviz_nbclust(datos_z[, c("gastoz", "educaz")], kmeans,
             method = "wss", k.max = 5) +
  labs(title = "Suma de cuadrados intra-agrupación (intra-clúster) (WSS)")
```
...
#| fig-width: 8
#| fig-height: 4.5
nb_hartigan <- NbClust(datos_z[, c("gastoz", "educaz")], distance = "euclidean",
                        min.nc = 2, max.nc = 4, method = "kmeans", index = "hartigan")

data.frame(k = 2:4, hartigan = nb_hartigan$All.index) |>
  ggplot(aes(x = k, y = hartigan)) +
  geom_line() + geom_point(size = 2) +
  labs(title = "Estadístico de Hartigan", x = "Número de clústeres k", y = "H(k)") +
  theme_bw()
```
...
#| fig-width: 8
#| fig-height: 4.5
nb_ch <- NbClust(datos_z[, c("gastoz", "educaz")], distance = "euclidean",
                  min.nc = 2, max.nc = 4, method = "kmeans", index = "ch")

data.frame(k = 2:4, ch = nb_ch$All.index) |>
  ggplot(aes(x = k, y = ch)) +
  geom_line() + geom_point(size = 2) +
  labs(title = "Índice de Calinski-Harabasz", x = "Número de clústeres k", y = "CH(k)") +
  theme_bw()
```
...
::: {.incremental}
1. Comenzar con tantos grupos como objetos ($n$ grupos). Las distancias entre grupos son las distancias entre los elementos originales.
...
| **Ward** | Mínimo incremento en la inercia intra-agrupación (intra-clúster) al fusionar | El más usado en la práctica |

## Ejemplo simulado: dendrograma {style="font-size: 85%;"}
...
plot(hc_complete, labels = datos$Individuo,
     main = "Agrupación jerárquica (clúster) (enlace completo)",
...
- **Criterio de inercia**: identificar el número de grupos $k$ a partir del cual la pérdida relativa de inercia inter-agrupación (inter-clúster) al pasar de $k$ a $k-1$ grupos es grande, es decir, al fusionar esos dos grupos quedan siendo menos diferentes entre sí. Se retiene el $k$ inmediatamente anterior a esa pérdida grande.
...
2. **Clasificación jerárquica** (Ward) sobre esas coordenadas, para construir el dendrograma y sugerir el número de agrupaciones (clústeres).
3. **Consolidación** de la partición con un algoritmo de $K$-medias, partiendo de los centros de gravedad obtenidos en el paso jerárquico, utilizando como valor de $K$ el número de agrupaciones (clústeres) que acaba de generar la clasificación jerárquica en el paso anterior.
:::
...
- `HCPC()` produce, además del dendrograma, una **caracterización automática** de cada agrupación (clúster): qué variables (o modalidades) lo tipifican.
:::
...
#| eval: false
library(pacman)
p_load(tidyverse, FactoMineR, factoextra, gt)
```
...
## Paso 3: Plano factorial por agrupación (clúster) {style="font-size: 88%;"}
...
fviz_cluster(res.clus.ecv, repel = TRUE,
             ...
             main = "Agrupación de departamentos (clúster) — Calidad de vida ECV 2022")
```

## Caracterización de las agrupaciones (clústeres) {style="font-size: 72%;"}

::: {.re-blue}
`leen_adultos`: % de adultos que saben leer y escribir. `inv_ipm`: inverso del IPM (100 − IPM). `inv_hacinamiento`: inverso del % de hogares en hacinamiento (100 − % hacinamiento).
:::
...
desc_ecv |>
  ...
  cols_label(variable = "Variable", `Mean in category` = "Media en agrupación (clúster)",
...
::: {.incremental}
- **Agrupación 1 (Clúster 1)** (4 dptos: Amazonas, Vaupés, Guainía, Vichada): valores **muy por debajo** de la media nacional en energía, alfabetización de adultos (`leen_adultos`), el indicador invertido de pobreza multidimensional (`inv_ipm`) y afiliación en salud. Es el grupo con las condiciones de vida más precarias — la Amazonía y la Orinoquía profundas.

- **Agrupación 2 (Clúster 2)** (13 dptos: Caribe, Pacífico, Arauca, Caquetá, Putumayo…): valores **por debajo** de la media en acceso a computador, internet, gas natural y educación superior — departamentos históricamente afectados por el conflicto y con menor cobertura de servicios.

- **Agrupación 3 (Clúster 3)** (16 dptos: Bogotá, Antioquia, Valle, eje cafetero, Santanderes…): valores **por encima** de la media en acceso a internet y computador, y en calidad de vivienda (paredes y piso adecuados) — la región Andina y las principales áreas metropolitanas.
:::

## Objetos "parangón" de cada agrupación (clúster) {style="font-size: 85%;"}

Son los departamentos **más representativos** (más cercanos al centro de su agrupación (clúster)):
...
## Contexto: Barómetro de las Américas {style="font-size: 78%;"}
...
## Plano de objetos coloreado por agrupación (clúster) {style="font-size: 88%;"}
...
## Caracterización cualitativa de las agrupaciones (clústeres) {style="font-size: 68%;"}
...
desc_lapop <- map_dfr(names(res.clus.lapop$desc.var$category), function(k) {
  as.data.frame(res.clus.lapop$desc.var$category[[k]]) |>
    ...
    mutate(cluster = k) |>
    select(cluster, modalidad, `Mod/Cla`, `Cla/Mod`, Global, v.test) |>
...
desc_lapop |>
  mutate(across(where(is.numeric), ~round(., 1))) |>
  gt(groupname_col = "cluster") |>
  cols_label(modalidad = "Modalidad", `Mod/Cla` = "% Mod/Cla", `Cla/Mod` = "% Cla/Mod",
...
::: {.incremental}
- **Agrupación 1 (Clúster 1)** (n=487, 36%): domina la modalidad **"Baja"** en las seis variables — ciudadanos con **desafección generalizada** hacia el sistema político.

- **Agrupación 2 (Clúster 2)** (n=126, 9%): grupo pequeño y atípico, tipificado casi exclusivamente por **"Respeto Media"** (concentra el 100% de esa modalidad); en el resto de variables se inclina hacia valores bajos. Es el tipo de perfil "particular" que un análisis exploratorio univariado difícilmente detectaría.

- **Agrupación 3 (Clúster 3)** (n=368, 27%): predominan las modalidades **"Media"** — ciudadanos **ambivalentes**, ni claramente críticos ni claramente leales al sistema.

- **Agrupación 4 (Clúster 4)** (n=385, 28%): domina la modalidad **"Alta"** en todas las variables — ciudadanos con **alta legitimidad percibida** del sistema político. Esta agrupación (clúster) tiene una proporción de **mujeres significativamente mayor** a la esperada ($\chi^2$, p = 0.022), asociación visible gracias a `Genero` como variable suplementaria.
:::
...
| 5 | 6813 | `ARWU_100_top.csv` (ranking mundial de universidades) | b, e, g, i, k |
...
3. Use `fviz_nbclust()` (WSS y silueta) para proponer un número de agrupaciones (clústeres) $k$.
...
8. Aplique un **ACP** sobre las mismas variables y luego `HCPC()` sobre el resultado. Compare el número de agrupaciones (clústeres) sugerido automáticamente con el que usted eligió en el punto 3.
9. Use `res.clus$desc.var$quanti` para **caracterizar** cada agrupación (clúster): ¿qué variables lo definen y en qué sentido (por encima o por debajo de la media)?
10. Nombre cada agrupación (clúster) con una etiqueta descriptiva y redacte un párrafo de interpretación.
:::
...
2. Aplique `HCPC()` sobre el resultado del ACM. ¿Cuántas agrupaciones (clústeres) sugiere automáticamente?
3. Grafique el dendrograma y el plano de objetos coloreado por agrupación (clúster).
4. Use `res.clus$desc.var$category` para identificar las modalidades que **tipifican** cada agrupación (clúster).
5. Agregue **ciudad** o **nivel educativo** (`Muestra_CC.csv`) como variable suplementaria en el ACM antes de correr `HCPC()`. ¿Alguna de estas variables se asocia significativamente con alguna agrupación (clúster)?
6. Elabore una **caracterización final**: nombre cada agrupación (clúster) según el perfil de justificaciones que lo tipifica.
:::
