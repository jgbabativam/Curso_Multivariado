...
## Análisis de las filas — Factores 1 y 2

```{r, echo=FALSE}
library(FactoMineR)
library(knitr)

coord <- acs_p16x17$row$coord[, 1:2]
cos2  <- acs_p16x17$row$cos2[, 1:2]
contrib <- acs_p16x17$row$contrib[, 1:2]
...
tabla_filas <- data.frame(
  ...
  Cos2_Dim1 = cos2[,1],
  Cos2_Dim2 = cos2[,2],
  Contrib_Dim1 = contrib[,1],
  Contrib_Dim2 = contrib[,2]
) |>
  mutate(across(-Fila, round, 3)) |>
  arrange(desc(Contrib_Dim1))

kable(tabla_filas, align = "c")
```
