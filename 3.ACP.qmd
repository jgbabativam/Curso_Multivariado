...
#| message: false
#| warning: false
# Generar dataset ECV si no existe
if (!file.exists(here::here("datos/ecv_departamentos_2022.csv"))) {
  source(here::here("datos_ECV_DANE.R"))
}
```
...
3. Guías para la interpretación
4. Construcción de las componentes
...
::: {.fragment}
::: {.re-blue}
> **Conclusión fundamental:** Ambos enfoques producen exactamente la misma solución. El TDVS garantiza la equivalencia mediante los valores y vectores propios de $X^\prime X$ y $XX^\prime$.
:::
:::
...
$$\text{med}(Y) = \frac{1}{n} Y^\prime \mathbf{1}_n = \mathbf{0}$$
...
::: {.fragment}
Los $\lambda_\alpha$ son los **valores propios** de $Y^\prime Y = R$, y las columnas de $V$ son sus **vectores propios**.
:::
...
**2. Covarianzas nulas (decorrelacionadas):**
...
$$\text{var}(Z_\alpha) = \frac{1}{n}\sum_{i=1}^n z_{i\alpha}^2 = \frac{1}{n}\,v_\alpha^\prime Y^\prime Y\, v_\alpha = \frac{\lambda_\alpha}{n} \cdot n = \lambda_\alpha$$
...
La coordenada de la variable $j$ sobre la componente $\alpha$:
...
Los vectores propios de $Y^\prime Y$ y $YY^\prime$ están profundamente relacionados.
...
Para las **variables**, la calidad en el plano formado por las componentes $\alpha$ y $\beta$ se mide con el coseno cuadrado:

$$\cos^2(\theta_{j,\alpha}) = \lambda_\alpha\, v_{j\alpha}^2$$

Una variable tiene buena representación en el plano 1-2 si $\cos^2_{j,1} + \cos^2_{j,2}$ es cercano a 1.
...
- Si $|r(X_j, Z_\alpha)|$ es alto, la variable $X_j$ está bien representada en la componente $\alpha$.
...
$$\text{CTR}(i, \alpha) = \frac{z_{i\alpha}^2}{\sum_{i^\prime=1}^n z_{i^\prime\alpha}^2} = \frac{z_{i\alpha}^2}{n\,\lambda_\alpha}$$
...
::: {.fragment}
::: {.callout-note}
En el biplot de FactoMineR (`fviz_pca_biplot`), los vectores de las variables están escalados por $\sqrt{\lambda_\alpha}$, lo que facilita la interpretación simultánea.
:::
:::
...
**Factor tamaño:**
Cuando la primera componente está correlacionada positivamente con **todas** las variables, se interpreta como un indicador de "tamaño" o "nivel general". Las componentes posteriores capturan contrastes (forma, estructura).
...
$$Z_\alpha = \widetilde{Y}\, v_\alpha = \sqrt{n}\; Y\, v_\alpha$$
...
# Ejemplo 1: Compresión de imágenes {background-color="#0077b6"}

## PCA aplicado a datos visuales
...
::: {.fragment}
Cada canal es una **matriz de intensidades** en $[0,1]$. Si aplicamos PCA a cada canal por separado:
...
# Cargar la imagen (853 × 1280 × 3 arreglo RGB)
imagen <- readJPEG(here::here("images/rushmore.jpg"))
cat("Dimensiones:", paste(dim(imagen), collapse = " × "), "\n")
cat("Filas (alto):", nrow(imagen), "| Columnas (ancho):", ncol(imagen),
    "| Canales RGB:", dim(imagen)[3])
```
...
#| echo: true
#| eval: false
# Submuestreo para reducir cómputo (cada 3 píxeles)
img <- imagen[seq(1, nrow(imagen), 3),
              seq(1, ncol(imagen), 3), ]

# PCA por canal (sin centrado: preserva la escala de intensidades)
pca_r <- prcomp(img[,,1], center = FALSE)
pca_g <- prcomp(img[,,2], center = FALSE)
pca_b <- prcomp(img[,,3], center = FALSE)
...
reconstruir <- function(pca_ls, n_comp) {
  sapply(pca_ls, function(p) {
    ...
    pmax(0, pmin(1, rec))          # clamping a [0, 1]
  }, simplify = "array")
}
```
...
if (exists("var_exp")) {
  df_var <- data.frame(
    comp = 1:length(var_exp),
    var_acum = var_exp * 100
  )
  puntos <- df_var[df_var$comp %in% c(3, 10, 30, 100), ]

  ggplot(df_var, aes(x = comp, y = var_acum)) +
    geom_line(color = "#0077b6", linewidth = 1.1) +
    geom_point(data = puntos, aes(x = comp, y = var_acum),
               color = "#C0392B", size = 3.5) +
    geom_text(data = puntos,
              aes(label = paste0(comp, " comp.\n",
                                 round(var_acum, 1), "%")),
              vjust = -0.5, size = 3.2, color = "#C0392B") +
    scale_x_continuous(limits = c(1, 150)) +
    labs(title = "Varianza acumulada — canal R (representativo)",
...
# Ejemplo 2: Rankings universitarios latinoamericanos {background-color="#0077b6"}
...
#| warning: false
library(pacman)
p_load(tidyverse, FactoMineR, factoextra, janitor)
...
# Promedios por país: columnas 2 (país) y 3-5 (rankings numéricos)
rank_sub <- rank_lat[-34, c(2:5)]          # 4 cols: país + 3 rankings
rank_paises <- aggregate(
  rank_sub[, 2:4],                          # solo columnas numéricas
  by  = list(Pais = rank_sub[, 1]),
  FUN = function(x) mean(x, na.rm = TRUE)
) |>
  column_to_rownames("Pais") |>
  setNames(c("SC_Lac", "QS", "WEB"))

# Eliminar países donde algún ranking es NaN
rank_paises <- rank_paises[complete.cases(rank_paises), ]

# Negativos: mayor valor = mejor posición
tres_rank <- -rank_paises
```
...
#| message: false
#| warning: false
pca_rank <- PCA(tres_rank, graph = FALSE)
```
...
#| fig-width: 9
#| fig-height: 4
fviz_eig(pca_rank, addlabels = TRUE, ylim = c(0, 100),
         barfill = "#0077b6", barcolor = "#0077b6",
         linecolor = "#C0392B") +
...
#| fig-width: 8
#| fig-height: 5
fviz_pca_var(pca_rank,
             col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE,
             title = "Plano 1-2 — Variables (rankings)") +
  theme_bw()
```
...
#| fig-width: 9
#| fig-height: 5.5
fviz_pca_ind(pca_rank,
             col.ind = "cos2",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE,
             title = "Plano 1-2 — Países (rankings universitarios)") +
  theme_bw()
```
...
#| fig-width: 9
#| fig-height: 5.5
fviz_pca_biplot(pca_rank,
                repel = TRUE,
                col.var = "#C0392B",
                col.ind = "#0077b6",
                title = "Biplot — Rankings universitarios latinoamericanos") +
  theme_bw()
```
...
#| fig-width: 9
#| fig-height: 4.5
# Bolivia=1, Cuba=3, Puerto Rico=10, Paraguay=13
pca_supl <- PCA(tres_rank, ind.sup = c(1, 3, 10, 13), graph = FALSE)
fviz_pca_biplot(pca_supl, repel = TRUE,
                col.var  = "#C0392B",
                col.ind  = "#0077b6",
                col.ind.sup = "#8E44AD",
                title = "Biplot con países ilustrativos (en morado)") +
  theme_bw()
```

# Ejemplo 3: Calidad de vida — ECV DANE {background-color="#0077b6"}
...
::: {.fragment}
El dataset que usaremos contiene **15 indicadores de calidad de vida** para los **33 departamentos** de Colombia (incluido Bogotá D.C.), construidos a partir de los resultados publicados por el DANE para la ECV 2022.
...
Los indicadores negativos (analfabetismo, hacinamiento, IPM) se transformaron como $100 - \text{indicador}$.
:::
:::
...
#| message: false
#| warning: false
ecv <- read.csv(here::here("datos/ecv_departamentos_2022.csv"))
rownames(ecv) <- ecv$departamento
ecv_num <- ecv |> select(-departamento)

cat("Dimensiones:", nrow(ecv_num), "departamentos ×",
    ncol(ecv_num), "indicadores\n")
cat("\nVariables:\n")
cat(paste0("  ", names(ecv_num)), sep = "\n")
```
...
corrplot(R_ecv,
         ...
         addCoef.col = "black",
         number.cex  = 0.45,
         title = "Matriz de correlaciones — Indicadores ECV 2022",
         mar = c(0, 0, 1.5, 0))
```

---

## Cálculo del ACP con FactoMineR
...
fviz_eig(pca_ecv,
         ...
         barcolor   = "#0077b6",
         linecolor  = "#C0392B") +
  geom_hline(yintercept = 100/ncol(ecv_num),
...
## Plano de variables (componentes 1 y 2)
...
#| fig-width: 9
#| fig-height: 5.5
fviz_pca_var(pca_ecv,
             col.var    = "cos2",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel      = TRUE,
             title      = "Plano 1-2 — Indicadores de calidad de vida") +
  theme_bw()
```
...
#| fig-width: 10
#| fig-height: 5.5
fviz_pca_ind(pca_ecv,
             col.ind   = "cos2",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel     = TRUE,
             labelsize = 3,
             title     = "Plano 1-2 — Departamentos de Colombia") +
  theme_bw()
```
...
#| fig-width: 10
#| fig-height: 5.5
fviz_pca_biplot(pca_ecv,
                repel     = TRUE,
                col.var   = "#C0392B",
                col.ind   = "#0077b6",
                labelsize = 3,
                title     = "Biplot — Calidad de vida ECV 2022") +
  theme_bw()
```
...
ggplot(coord_var, aes(x = Dim1, y = Dim2, label = Variable)) +
  ...
  labs(title = "Coordenadas de variables en el plano 1-2",
       x = paste0("Dim 1 — Nivel general de calidad de vida (",
                  round(eig_ecv[1,2], 1), "%)"),
       y = paste0("Dim 2 — Conectividad vs. Servicios básicos (",
                  round(eig_ecv[2,2], 1), "%)")) +
  theme_bw()
```
...
## Lectura del plano: regiones de Colombia

::: {.incremental style="font-size: 0.88em;"}
- **Dim 1 (eje horizontal):** captura el **nivel general de calidad de vida**. Bogotá D.C. y departamentos del Eje Cafetero tienen coordenadas altas (derecha); Chocó, La Guajira y territorios amazónicos están a la izquierda.
- **Dim 2 (eje vertical):** diferencia entre departamentos con **alta conectividad** (internet, computador, gas natural — arriba) vs. **alta cobertura de servicios básicos** (acueducto, alcantarillado — abajo). San Andrés, por ejemplo, tiene buena cobertura de servicios pero baja en gas natural.
- **Variables en el mismo cuadrante:** están correlacionadas positivamente (p.ej., `internet` y `computador`).
- **Variables en cuadrantes opuestos:** están negativamente correlacionadas en ese plano.
:::
...
## Plano de variables: criterios activos vs. rankings ilustrativos
...
## Laboratorio (1/3) {style="font-size: 90%;"}

Usando los datos de la **Encuesta Nacional de Calidad de Vida (ECV 2022)** cargados en la presentación (`ecv_departamentos_2022.csv`):

::: {.incremental}
1. Calcule la **matriz de correlaciones** entre los 15 indicadores. Identifique los tres pares de indicadores más correlacionados y los tres con correlación más cercana a cero.

2. Aplique el ACP usando `PCA()` de `FactoMineR`. Compare los resultados usando:
   a. `scale.unit = TRUE` (sobre $R$)
   b. `scale.unit = FALSE` (sobre $S$)

   ¿Cambian los resultados? ¿Por qué?

3. ¿Cuántas componentes retiene cada uno de los tres criterios presentados? Justifique cuál le parece más adecuado para este conjunto de datos.
:::

---

## Laboratorio (2/3) {style="font-size: 90%;"}

::: {.incremental}
4. Construya el **plano 1-2 de variables** con `fviz_pca_var()`. Identifique:
   - Las dos variables con mayor contribución a la Dimensión 1.
   - Las dos variables con mayor contribución a la Dimensión 2.
   - Una variable con bajo cos² en el plano 1-2 (mal representada).

5. Construya el **plano de individuos** con `fviz_pca_ind()` coloreando por cos². Identifique:
   - El departamento con mejor calidad de vida general.
   - El departamento peor representado en el plano 1-2.
   - Dos departamentos con perfiles similares (cercanos en el plano).
   - Dos departamentos con perfiles opuestos (en cuadrantes contrarios).

6. Genere el **biplot** e interprete qué variable(s) caracterizan al departamento que quedó en el extremo derecho del eje 1.
:::

---

## Laboratorio (3/3) {style="font-size: 90%;"}

::: {.incremental}
7. Agregue la variable `ipm` original (sin transformar) como **variable suplementaria** usando `quanti.sup`. ¿Cómo se ubica en el plano de variables? ¿Qué componente captura mejor la pobreza multidimensional?

8. Repita el ACP con los indicadores de **vivienda** (`piso_adec`, `paredes_adec`) como variables suplementarias. ¿Cambia la interpretación de las componentes?

9. **Reto:** Clasifique los departamentos en **4 grupos** usando K-means sobre las coordenadas de las 2 primeras componentes. Visualice los grupos en el biplot con `fviz_cluster()`. ¿Coinciden los grupos con las regiones naturales de Colombia?

10. Con los datos de **ciudades colombianas** disponibles en el repositorio, realice un ACP con las variables de infraestructura e investigación. Use la variable `Región` como variable ilustrativa cualitativa. Utilice `quali.sup` en `PCA()`.
:::
