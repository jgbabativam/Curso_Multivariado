...
## Calidad de la representación {style="font-size: 90%;"}
...
## Correlaciones variable-factor {style="font-size: 90%;"}
...
## Contribuciones a la varianza {style="font-size: 90%;"}
...
## Cosenos cuadrados {style="font-size: 90%;"}
...
# Ejemplo 1: Análisis de imágenes {background-color="#0077b6"}
...
if (exists("var_exp")) {
  ...
  ggplot(df_var, aes(x = comp, y = var_acum)) +
    ...
    geom_text(data = puntos,
              ...
              vjust = -0.5, size = 3.2, color = "#C0392B") +
    scale_x_continuous(limits = c(1, 150)) +
    labs(title = "Varianza acumulada — canal R (del RGB)",
         x = "Número de componentes principales",
         y = "Varianza explicada acumulada (%)") +
    theme_bw()
}
...
#| warning: false
R_ecv <- cor(ecv_num)
corrplot(R_ecv, method  = "color", type    = "upper", order   = "hclust", tl.cex  = 0.65, tl.col  = "black", addCoef.col = "black", number.cex  = 0.45, mar = c(0, 0, 1.5, 0))
```
